...
def refresh_markers(
    viewer, points, radius=0.005, rgba=(1.0, 1.0, 0.0, 1.0), ngeom_start=0
):
    ...
    def _on_step(self) -> bool:
        ...
        for info in infos:
            if "episode" in info:
                ...
                self.logger.record(
                    "rollout/ep_ee_tracking_error", info["ee_tracking_error"]
                )
...
class UpdateCheckpointCallback(BaseCallback):
    ...
    def __init__(
        self, save_path=None, name_prefix="model", save_freq_updates=10, verbose=0
    ):
