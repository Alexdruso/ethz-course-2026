...
if __name__ == "__main__":
    ...
    if args.num_envs > 1:
        ...
        envs = SubprocVecEnv(
            [make_env() for _ in range(args.num_envs)], start_method=start_method
        )
    ...
    print(
        f"Training for {total_update_steps} iterations (~{total_timesteps} steps, rollout size {rollout_batch_size})"
    )
        ...
        model.learn(
            ...
            callback=[
                ...
                KLAdaptiveLRCallback(
                    target_kl=0.05, init_lr=1e-3, min_lr=1e-5, max_lr=1e-3
                ),
            ],
        )
    finally:
